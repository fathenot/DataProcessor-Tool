    ...
    public partial class Series<DataType> : ISeries<DataType> where DataType : notnull
    {
        ...
        public class View
        {
            ...
            public Series<DataType> ToSeries(string? name = null)
            {
                ...
                foreach (var idx in this.convertedToIntIdx)
                {
                    data.Add(this.series.values[idx]);
                }
                return new Series<DataType>(name, data, this.index);
            }

            public void UpdateValue(object idx, DataType newValue)
            {
                if (!index.Contains(idx))
                    throw new KeyNotFoundException($"Index:{idx} not in View");
